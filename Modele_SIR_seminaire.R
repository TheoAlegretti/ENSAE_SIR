...
#on produit une pluralité de combinaison de b dans 0 jusqu'à 2 et g dans 0 jusquà 1. 
...
#(ii) R_0 < 1 <=> beta < gamma : l'épidémie ne décolle pas 
test_parameter(0.7,0.9)

#(iii) R_0 > 1 <=> beta > gamma : l'épidémie explose puis descend rapidement car l'entièreté de la population est infecté très vite 
...
#test avec un jeu de paramètre mauvais : b =g  = 0.7 

vraisemblance(0.7,0.7) # = 0 (inf à 2.636124e-25) (c'est logique mais la différence est très faible)

#On calcul la distribution à posteriori en utilisant les fonctions dunif pour les densités à priori des paramètres et et la fonction de vraisemblance au dessus ! 
#on ignore la constante de normalisation en temps normal c'est une valeur à une constante près
distrib_post <-function(b,g){
  post <- dunif(b,min=0,max=10)*dunif(g,min=0,max=1)*vraisemblance(b,g)
  return(post)
}
#avec notre jeu de paramètre : 2.636124e-26
distrib_post(1.7,0.44)

#question 3 : Algorithme MCMC (Metropolis-Hasting)
#initiation b= 1.5 et g = 0.5 et N = 10 000 (nb de simulation)
test_parameter(1.5,0.5) #on est pas mal en apparence (on devrait converger vite)
#on va construire une matrice de dimension (Nx3), troisième colonne étant le postiriori
#et le posterie de première itération

#on tire dans une loi normale avec valeur absolue pour beta et normale tronquer pour g 
...
thetas = MCMC_metropolis_hasting(1.3,0.2,1000,0.3,0.09)

#on peut jouer aussi bien sur les paramètres de bases que sur la valeur des variances des loi instrumentales 

#show the best parameters => b = 1.65, g = 0.48
thetas[N,]
#let's plot it 
...
# pour cela on va enregistrer le taux de succès dans une nouvelle variables
# on va simuler plusieurs sigma different pour trouver la valeur ou le taux de succès et égale à 1/4 
...
#on lance des combinaison différentes de sigma (sigma_beta de 0.05 jusqu'à 0.5 et sigma_gamma de 0.05 jusqu a 0.2)

sigma_opti <- matrix(nrow = length(seq(0.05,0.5,by=0.05))*length(seq(0.05,2,by=0.05)), ncol = 3)
i = 1 
for (s1 in seq(0.05,0.5,by=0.05)){
  for (s2 in seq(0.05,2,by=0.05)){
    print(paste("sigma_b =",as.character(s1),'and sigma_g = ',as.character(s2)))
    sigma_opti[i,1] = s2
    sigma_opti[i,2] = s1
    thetas = MCMC_metropolis_hasting(1.5,0.5,100,s1,s2)
    p = proba_succes(thetas)
    sigma_opti[i,3] = p
    i = i +1 
  }}
...
sigma_opti <- matrix(nrow = length(seq(0.01,0.2,by=0.005))*length(seq(0.01,0.2,by=0.005)), ncol = 3)
i = 1 
for (s1 in seq(0.01,0.2,by=0.005)){
  for (s2 in seq(0.01,0.2,by=0.005)){
    print(paste("sigma_b =",as.character(s1),'and sigma_g = ',as.character(s2)))
    sigma_opti[i,1] = s2
    sigma_opti[i,2] = s1
    thetas = MCMC_metropolis_hasting(1.5,0.5,2000,s1,s2)
    p = proba_succes(thetas)
    sigma_opti[i,3] = p
    i = i +1 
  }}
...
sigma_opti[400:550,3]>0.25 
sigma_opti[431,] #bingo 
...
#introduction d'une durée d'incubation dans le modèle SIR => SEIR

#Je reprend le code de l'énoncé : 
...
time = c(0:50)
#on introduit le nouvel état (E) dans le nombre d'infecté et les nombres de périodes 
#logique vu que chaque infecté passe 2 jours dans l'état E avant de rentrer dans I.
...
#on remarque que  l'introduction de zeta, le pique de l'épidémie est bien plus lointain et est moins intense car il est dillué par la guérison qui intervient durant la latence. 
